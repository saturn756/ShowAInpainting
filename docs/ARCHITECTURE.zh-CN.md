...
```
┌───────────────────────────── 用户浏览器 ─────────────────────────────┐
│  Vue3 单页应用 (激活码 → 上传 → 画蒙版/框选 → 生成 → 结果解密展示)      │
│  · WebCrypto：上传用站点公钥加密 / 结果用用户私钥解密                    │
│  · 用户私钥只存浏览器 localStorage                                     │
└───────┬──────────────────────────────┬───────────────────────────────┘
        │ HTTPS 同源 (your-domain.example) │ 直传密文 (无浏览器 CORS 问题)
        │ 全走 Nginx → 逻辑层           │
        ▼                              ▼
┌─────────────────┐          ┌─────────────────────┐
│  公网逻辑服务器    │  SSH隧道  │    OSS 对象存储       │
│  Nginx (443)     │ 19944↔7861 │  加密对象存储       │
│  ┌────────────┐  │ ←──────── │                    │
│  │ 逻辑层      │  │  仅 JSON  │  · 只存密文          │
│  │ FastAPI 8000│──┼──┐       │  · 阿里云无法解密     │
│  │ 激活/队列/OSS│  │  │       └─────────┬───────────┘
│  │ 静态前端 dist │  │  │                 │ 下载密文 + 上传密文
│  └────────────┘  │  │                 ▼
└─────────────────┘  │   ┌──────────────────────────────┐
